...
import { getContractDeployedCode } from "./helpers/utils";
...
describe("Safe with Guard", function () {
    ...
    it("EvenValueGuard should allow transactions with even values", async function () {
        // Extract the runtime bytecode
        const runtimeBytecode = await getContractDeployedCode("EvenValueGuard");
        ...
        const enableGuardData = encodeFunctionData({
            ...
            args: [easyGuardAddress, runtimeBytecode, false]
        });
        ...
        expect(checkerProgram).to.be.equal(runtimeBytecode);
    ...
    it("EvenValueGuard should REJECT transactions with odd values", async function () {
        // Extract the runtime bytecode
        const runtimeBytecode = await getContractdeployedCode("EvenValueGuard");
